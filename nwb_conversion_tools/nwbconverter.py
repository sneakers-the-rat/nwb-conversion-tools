...
class NWBConverter:
    ...
    def run_conversion(
        ...
        ) or nwbfile is None, "Either pass a nwbfile_path location with save_to_file=True, or a nwbfile object, but not both!"
        if conversion_options is None:
            conversion_options = dict()
        else:
            validate(
                instance=conversion_options, schema=self.get_conversion_options_schema()
            )
        ...
        if nwbfile is None:
            ...
            if "Subject" in metadata:
                ...
                if "date_of_birth" in metadata["Subject"] and isinstance(
                    metadata["Subject"]["date_of_birth"], str
                ):
        ...
        if save_to_file:
            ...
            with NWBHDF5IO(nwbfile_path, mode=mode) as io:
                ...
                for (
                    interface_name,
                    data_interface,
                ) in self.data_interface_objects.items():
                    data_interface.run_conversion(
                        nwbfile,
                        metadata,
                        **conversion_options.get(interface_name, dict()),
                    )
            ...
            for interface_name, data_interface in self.data_interface_objects.items():
                data_interface.run_conversion(
                    nwbfile, metadata, **conversion_options.get(interface_name, dict())
                )
            return nwbfile
