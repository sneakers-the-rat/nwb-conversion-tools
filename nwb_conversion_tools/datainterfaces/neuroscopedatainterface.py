            ...
            Ecephys=dict(
                ...
                Electrodes=[
                    dict(
                        ...
                        data=[
                            x
                            for channels in shank_channels
                            for x, _ in enumerate(channels)
                        ],
                    ),
                    dict(
                        name="group_name",
    ...
    def get_metadata(self):
        ...
        metadata["Ecephys"].update(
            ElectricalSeries=dict(
                name="ElectricalSeries", description="Raw acquisition traces."
            )
        )
...
class NeuroscopeMultiRecordingTimeInterface(BaseRecordingExtractorInterface):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.subset_channels = get_shank_channels(
            xml_file_path=get_xml_file_path(
                data_file_path=self.source_data["folder_path"]
            ),
            sort=True,
        )
