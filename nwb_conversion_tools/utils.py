...
def get_schema_from_hdmf_class(hdmf_class):
    ...
    pynwb_children_fields = [
        f["name"] for f in hdmf_class.get_fields_conf() if f.get("child", False)
    ]
    ...
    for docval_arg in docval["args"]:
        ...
        # type string
        elif docval_arg["type"] is str or (
            isinstance(docval_arg["type"], tuple) and str in docval_arg["type"]
        ):
        ...
        # if PlaneSegmentation, skip it
            ...
            isinstance(docval_arg["type"], tuple)
            and pynwb.ophys.PlaneSegmentation in docval_arg["type"]
        ):
            continue
